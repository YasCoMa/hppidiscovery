...
class DataPreprocessing:
    def __init__(self, base_dir, network, method, target_taxon):
        self.base=base_dir
        self.hpnet=network
        self.method=method
    ...
    def ___feed_taxon_mapping(self, tquery, tok):
        self.string_mapp_taxon[tquery]=tok
        with open('mapping_taxons_ok_string.tsv','a') as gf:
            gf.write("%s\t%s\n" %(tquery, tok) )
            
    
    def __check_taxonomy_string(self, original):
        if(original in self.string_mapp_taxon.keys()):
            return True, self.string_mapp_taxon[original]
        else:
            proteins={}
            flag=True
            taxon_ok=False
            ...
            while(flag):
                try:
                    ...
                    self.loaded_taxons.add(t)
                    self.___feed_taxon_mapping(original, t)
                except:
                    t=lineage[(-1)*j]
                    if(j==3):
                        flag=False
            ...
            if (len(proteins)>0):
                with open(self.base+"string_entries.tsv","a") as g:
    ...
    def _parse_combined_result(self, df):
        ...
        if(not self.target_taxon in self.loaded_taxons):
            flag, taxont=self.__check_taxonomy_string(self.target_taxon)
        else:
            taxont=self.string_mapp_taxon[self.target_taxon]
                ...
                if(p1 != p2):
                    ...
                    if(len(taxid)>0):
                        ...
                        if(not taxon1 in taxons.keys()):
                            flag=True
                            if(not taxon1 in self.loaded_taxons):
                                flag, taxon=self.__check_taxonomy_string(taxon1)
                            if(flag):
                                if(not taxon1 in self.string_mapp_taxon.keys()):
                                    self.string_mapp_taxon[taxon1]=taxon1
                                taxons[taxon1]=self.string_mapp_taxon[taxon1]
                            else:
                                taxons[taxon1]=None
                    ...
                    if(len(taxid)>0):
                        ...
                        if(not taxon2 in taxons.keys()):
                            flag=True
                            if(not taxon2 in self.loaded_taxons):
                                flag, taxon=self.__check_taxonomy_string(taxon2)
                            if(flag):
                                if(not taxon2 in self.string_mapp_taxon.keys()):
                                    self.string_mapp_taxon[taxon2]=taxon2
                                taxons[taxon2]=self.string_mapp_taxon[taxon2]
                            else:
                                taxons[taxon2]=None
                                flag_tax=flag_tax and False  
                        else:
        ...
        if (len(proteins)>0):
            with open(self.base+"hpidb_entries.tsv","a") as g:
                for p in proteins.keys():
                    g.write("%s\n" %(p) )
    ...
    def _parse_mitab(self, df):
        ...
        proteins={}
        pairs=set()
        df=df[ ['protein_xref_2', 'protein_xref_1', 'protein_taxid_2', 'protein_taxid_1'] ]
        g=open(self.base+'hpidb_pos.tsv', 'w')
        for i in range(len(df)):
            p1=df.iloc[i, 0]
            p2=df.iloc[i, 1]
    ...
    def parse_hpidb_results(self):
        f=open(self.base+"string_entries.tsv","w")
        f.close()
        
        f=open(self.base+"hpidb_entries.tsv","w")
        f.close()
                    ...
                    for line in text:
                        ...
                        if( len(l)>1 ):
                            if(source_db=='STRING'):
                                ...
                                with open('mapping_string_uniprot.tsv') as g:
                                    g.write('%s\t%\t%s\t%s\n' %(l[0], l[1], l[3], identifiers[l[0]] ) )
                    check=True
                except:
                    check=False
    ...
    def _get_annotations_string(self):
        ...
        string_ids={}  
        #for g in genome_folders:
            #f=open(main_folder+g+'/string_entries.tsv','r')
        f=open(self.base+'string_entries.tsv','r')
        for line in f:
            l=line.replace('\n','').split('\t')
    ...
    def _get_annotations_hpidb(self):
        ...
        hpidb_ids=set()       
        #for g in genome_folders:
            #f=open(main_folder+g+'/hpidb_entries.tsv','r')
        f=open(self.base+'hpidb_entries.tsv','r')
        for line in f:
            l=line.replace('\n','')
    ...
    def handle_singleOrganism_interactions_string_mapping(self):
        ...
        for taxon in taxon_interactions.keys():
            if( not os.path.isfile('taxon_interactions/'+taxon+'_'+group+'.tsv') ):
